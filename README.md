...
![Data ISC — import a dataset, visualise trajectories, run what-if scenarios, compare futures, race the rankings, and export to social video](docs/demo.svg)
...
- **What-if scenarios** — start from the **🦢 Black Swan preset library** (one-click economic shocks like an automation wave, fertility crisis, decoupling, pandemic, or energy shock that auto-disperse across your entities), or build structured scenarios with per-entity rates, time-phased impacts, and value ceilings — no AI key required. Connect your own AI provider for causal per-entity breakdowns: for **imported datasets**, describe a scenario in plain language and AI returns a per-entity impact table you review, edit, and confirm before anything is applied; for the **GDP demo**, describe any shock and AI generates and applies the full parameter set instantly. Import/export scenarios as JSON for sharing and version control.
...
- **Import datasets** — click **Generate & review**, describe a scenario in plain language (e.g. *"a supply chain disruption cuts growth 3%/yr for 2 years"*), and a review modal opens showing every affected entity with its AI-generated `Δ%/yr`, year window, and a one-line rationale. Edit any value, uncheck entities to exclude, rename the scenario — then click **Create scenario** to confirm. Nothing is applied until you confirm.
