...
- **What-if scenarios** — start from the **🦢 Black Swan preset library** (one-click economic shocks that auto-disperse across your entities), or build structured scenarios with per-entity rates, time-phased impacts, and value ceilings — no AI key required. Connect an AI provider for a fully structured pipeline: prompt → AI maps to dataset schema (variables, units, ranges, granularity) → validator checks (variable existence, bounds, conflicts, unit normalization) → review modal showing baseline → Δ%/yr → projected per entity → confirm. Nothing is applied until you confirm. Import/export scenarios as JSON for sharing and version control.
